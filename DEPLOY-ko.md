...
한국어 페이지는 **Pretendard** 한 벌로 통일했습니다(제목·본문 모두). jsDelivr에서
동적 서브셋으로 받아오기 때문에 페이지에 실제로 쓰인 음절만 내려받습니다. CDN이
막힌 환경에서는 시스템 한글 폰트(애플 SD 산돌고딕 Neo / 맑은 고딕)로 떨어지므로
글자가 깨지지는 않습니다. 영문 페이지는 기존대로 Spectral + IBM Plex 조합입니다.

작은 라벨의 한글이 자간이 벌어져 보이던 문제도 잡았습니다. 모노스페이스 폰트에는
한글 글리프가 없어서 시스템 폰트로 떨어지고, 거기에 자간까지 붙어 있었습니다.
이제 숫자·영문만 모노로 나오고 한글은 Pretendard로 나옵니다.

### 전문용어 표기

한국어 페이지에서도 **현장에서 영어로 쓰는 용어는 영어 그대로** 둡니다 —
`Burn-in`, `Light soaking`, `self-healing`, `proton irradiation`,
`transport layer`, 논문 태그, 프로젝트 태그. 연결하는 문장만 한국어입니다.
그래프 축 라벨도 영문입니다.

### 메인 페이지 스크롤 섹션

메인 상단은 어두운 배경의 한 덩어리(히어로 + 열화 곡선 스토리)이고, 그 아래부터
평소 테마로 돌아옵니다. 상세 페이지는 일부러 차분하게 뒀습니다 — 스크롤에 따라
움직이는 요소가 하나도 없습니다.

스크롤 섹션은 `src/components/StoryScroll.astro` 한 파일입니다. 스크롤 진행도가
MPPT 시간축에 대응해서, 곡선이 그 시점까지 그려지고 숫자가 올라가며 해당 구간
설명이 밝아집니다.

| 바꾸고 싶은 것 | 위치 |
| --- | --- |
| 네 구간의 제목·설명 | `src/i18n/ui.ts` 의 `home.story.stages` |
| 히어로 숫자 4개 | `src/i18n/ui.ts` 의 `home.stats` |
| 스크롤 길이 | `StoryScroll.astro` 의 `.story-track { height: 340vh }` |
| 구간 경계(시간) | 같은 파일의 `bounds = [0, 60, 320, t80]` |

접근성은 지켜 뒀습니다. 자바스크립트가 꺼져 있거나 OS에서 "동작 줄이기"를 켠
사용자에게는 곡선이 처음부터 완성된 상태로, 네 구간 설명이 전부 또렷하게 보입니다.
스크롤을 해야만 읽을 수 있는 글자는 하나도 없습니다.
